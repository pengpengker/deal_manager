...
def GetDealInfo(session: httpx.Client, Domain: str, deal_id: str):
    ...
    for item in resp_json["viewModel"]["multiParentItemList"]["items"]:
        ...
        if not item["children"]:
            result_json["items"].append({"asin": item["asin"], "price": item["dealPrice"]["value"], "quantity": item["sellerQuantity"], "sku": item["sku"]})
        for childrenitem in item["children"]:
            table.add_row(item["asin"], childrenitem["asin"], childrenitem["sku"], "子体")
            result_json["items"].append({"asin": childrenitem["asin"], "parentAsin": item["asin"], "price": childrenitem["dealPrice"]["value"], "quantity": childrenitem["sellerQuantity"], "sku": childrenitem["sku"]})
    console.print(table)
    return result_json
...
def main():
    try:
        ...
        while True:
            ...
            action = Prompt.ask("Enter your action", choices=["add", "remove", "show"], default="show")
            if action == "show":
                DealInfo = GetDealInfo(session=session, Domain=Domain, deal_id=DealId)
            elif action == "add":
                ...
                while True:
                    c_item = {}
                    parentAsin = Prompt.ask(prompt="如果这是个子体则输入父体ASIN,没有则留空直接跳过", default="")
                    if parentAsin:
                    ...
                    action = Prompt.ask("是否提交?", choices=["yes", "no", "revoke"], default="yes")
                    if action == "yes":
                        break
                    elif action == "revoke":
